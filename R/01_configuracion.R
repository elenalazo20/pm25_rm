...
suppressPackageStartupMessages({
  library(here); library(fs); library(tidyverse)})
...
tree <- capture.output(dir_tree(here(), recurse = TRUE))
writeLines(tree, here("output/estructura_proyecto.txt"))
writeLines(c("```", tree, "```"), here("output/estructura_proyecto.md"))

message("Estructura creada ✅ y árbol guardado en output/estructura_proyecto.{txt,md}")
